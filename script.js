...
function createBoard(statusName, status) {
  ...
  document.getElementById(id).addEventListener("drop", (event) => {
    ...
    tempArr.push(dragMeCard);
    switch (tempStatus) {
      case "toDo":
        toDoCards.splice(dragMeCard, 1);
        break;
      case "doing":
        doingCards.splice(dragMeCard, 1);
        break;
      case "stuck":
        stuckCards.splice(dragMeCard, 1);
        break;
      case "done":
        doneCards.splice(dragMeCard, 1);
        break;
    }
    dragMeCard.status = newStatus;
    drawCards(tempStatus);
    drawCards(newStatus);
    countCards(tempStatus);
    countCards(newStatus);
  });
  document.getElementById(id).addEventListener("dragover", (event) => {
    event.preventDefault();
  });
...
//     document.getElementById(id).appendChild(dragMe);
//   });
